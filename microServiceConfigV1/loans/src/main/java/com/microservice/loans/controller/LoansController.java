...
import com.microservice.loans.dto.LoansConfigurationDto;
...
public class LoansController {
        ...
        @Autowired
        private LoansConfigurationDto loansConfigurationDto;
        ...
        @Operation(summary = "Fetch Loan Config Details REST API", description = "REST API to fetch loan config details based on a mobile number")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "HTTP Status OK"),
                        @ApiResponse(responseCode = "500", description = "HTTP Status Internal Server Error", content = @Content(schema = @Schema(implementation = ErrorResponseDto.class)))
        })
        @GetMapping("/fetchloanConfig")
        public ResponseEntity<LoansConfigurationDto> fetchLoanConfig() {
                return ResponseEntity.status(HttpStatus.OK).body(loansConfigurationDto);
        }
